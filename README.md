...
`scripts/dev.sh` binds Uvicorn to `0.0.0.0` by default so the dev server is
also reachable at `http://<your-LAN-IP>:8000` for on-device mobile testing
(see `docs/qa/antigravity-mobile-qa-prompt.md`). Override with
`HOST=127.0.0.1 bash scripts/dev.sh` for loopback-only.
