...
- `.socratink-brain/`
  Socratink Brain product-memory knowledge base. This is storage, not an agent.
...
- `docs/codex/session-bootstrap.md`
  Compatibility shim for older instructions; the canonical bootstrap remains `docs/codex/onboarding.md`.
...
- If older instructions mention `docs/codex/session-bootstrap.md`, treat it as an alias to `docs/codex/onboarding.md`, not a separate source of truth.
- In mixed-agent workflows, `socratinker` owns final consolidation of `docs/project/state.md`.
...
- Socratink Brain is the knowledge-base substrate and the `socratink-brain` skill is the maintenance interface.
  Do not describe Socratink Brain as the executor; `socratinker` owns execution.
- Resolve specialist disagreement with an explicit decision record.
  ...
  Use `glenna` to formalize significant decisions into `docs/codex/decision-log.md` using the MADR template.
...
- Do not assume `rob` is the default executor.
  The repo default agent is `socratinker`, which owns direct execution unless a specialist is explicitly better suited.
