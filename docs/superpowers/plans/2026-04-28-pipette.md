...
> **B-Revision (2026-04-28, commit `ef83756`).** This plan was originally locked at the v1 design (28 rounds of gemini sanity-checks + ChatGPT audit). After implementation completed and codex passed all 4 milestones, the user opted to swap the Step 1 skill from `grill-me` to `grill-with-docs` (mattpocock's variant). Because `grill-with-docs` updates the project glossary inline during grilling, **Step 1.5 was collapsed into Step 1**. Concrete changes:
>
> - **Skill swap:** Step 1 invokes `Skill: grill-with-docs` (was `grill-me`).
> - **Step 1.5 removed:** the `ubiquitous-language` skill is no longer called as a separate step. Wherever this plan describes "Step 1.5" or `01b-glossary-delta.md` below, treat as pre-revision context.
> - **Glossary file rename:** `docs/pipeline/_meta/UBIQUITOUS_LANGUAGE.md` → `docs/pipeline/_meta/CONTEXT.md`. Single source of truth; updated in-place by grill-with-docs during Step 1.
> - **Per-feature glossary delta gone:** `01b-glossary-delta.md` is no longer produced; the glossary lives only in `_meta/CONTEXT.md`.
> - **State-machine consequences:** `jump_back_to ∈ {1, 2}` (was `{1, 1.5, 2}`). `gate_glossary` and `step_1_5_glossary` nodes dropped from `pipeline_graph.json`. Doctor's `_verify_ubiquitous_language_skill` Check dropped from `CHECKS`; replaced with `_verify_grill_with_docs_skill`.
> - **Live source of truth:** `tools/pipette/` Python modules + `.claude/commands/pipette.md`. The plan and spec describe the original v1 design; live code is authoritative.

**Goal:** Ship `/pipette <topic>`, a project-only slash command that drives a multi-step heavy-planning pipeline (graph reconnaissance → grill-with-docs (glossary updated inline) → diagram → multi-agent sanity gate → plan → subagent execute → no-mistakes-gated push → eval) with deterministic gates, lockfile-based mutual exclusion, NEEDS_RESEARCH pause/resume, and per-feature artifact folders committed to git.

**Architecture:** Two halves — (1) a markdown slash command at `.claude/commands/pipette.md` that scripts the *running Claude session* to invoke skills (`grill-with-docs`, `superpowers:writing-plans`, `superpowers:subagent-driven-development`) and the `code-review-graph` MCP step-by-step; (2) a deterministic Python helper module at `tools/pipette/` that the slash command shells into for everything that must NOT be left to LLM judgment — lockfile state machine, atomic folder rename on abort/crash, anti-loop counters, gemini YAML schema parsing, SubagentStop hook handler, preflight checks. Artifacts land in `docs/pipeline/YYYY-MM-DD-HHMMSS-<topic>/` and are committed; cross-feature learning happens via Step 0 scanning prior `00-graph-context.md` files.

**Tech Stack:** Python 3.13 (pyenv), pytest, pydantic v2, PyYAML, `code-review-graph` CLI/MCP, `gemini` CLI, `no-mistakes` git proxy, hand-rolled pipeline-graph structural validator (replaces the spec's reference to "Agentproof" — see §1 Dependency reality for why), Claude Code skills (`grill-with-docs`, `workflow-diagramming`, `whiteboarding`, `superpowers:*`), bash for thin shell wrappers, `code-review-graph` MCP already wired in this repo.

**Source spec:** [docs/superpowers/specs/2026-04-28-pipette-design.md](../specs/2026-04-28-pipette-design.md) — locked at commit `dd90e50` (v1 design). The spec also has its own B-revision note at the top.
...
| `ubiquitous-language` skill present | ✅ `~/.claude/skills/ubiquitous-language/SKILL.md` (writes to `UBIQUITOUS_LANGUAGE.md` in CWD by default) | **DEPRECATED in B-revision (2026-04-28).** Step 1.5 was collapsed into Step 1; `grill-with-docs` (replaces `grill-me`) updates the glossary inline at `_meta/CONTEXT.md`. The ubiquitous-language skill is no longer invoked. |
| `grill-with-docs` skill present (B-revision) | ⚠ Forked from mattpocock's skills repo. Symlink `/path/to/matt_skills/skills/engineering/grill-with-docs` → `~/.claude/skills/grill-with-docs/`. | Used at Step 1; reads/writes `_meta/CONTEXT.md` inline; may emit ADRs at `docs/adr/` lazily. |
...
│   ├── CONTEXT.md                        # NEW — seed; project DDD glossary, updated inline by grill-with-docs (was UBIQUITOUS_LANGUAGE.md pre-B-revision)
...
_ALLOWED_JUMP = (1, 2)
# B-revision (2026-04-28): Step 1.5 collapsed into Step 1; jump_back_to=1.5
# is no longer valid. Was (1, 1.5, 2) in the v1 design.
...
- `glossary.md`: checks every domain term in grill + diagram against `_meta/CONTEXT.md` (B-revision: was `_meta/UBIQUITOUS_LANGUAGE.md`); flags synonyms and undefined terms.
