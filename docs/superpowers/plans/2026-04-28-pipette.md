...
The handler is implemented in Task **C7** (it depends on `lockfile`, `trace`, `sanity/schema`). This task installs the **wiring** — settings.json entry pointing to the script — and is a stub at first. Wiring before code so the handler is exercised by orchestrator tests as it's built.

**Files:**
- Create: `tools/pipette/subagent_stop.py` (stub returning `{"permissionDecision": "allow"}`; full impl in Task **C7**)
- Modify: `.claude/settings.json` (add SubagentStop hook block)
...
- [ ] **Step B3.c: Modify `.claude/settings.json`**

Add to the existing `hooks` object (alongside `PostToolUse` and `SessionStart`):
...
Use the `update-config` skill via Skill tool to write this — DO NOT hand-edit `.claude/settings.json` to avoid breaking the existing PostToolUse and SessionStart blocks. The skill is invoked by the implementing agent with the prompt: *"Add a SubagentStop hook entry to project `.claude/settings.json` running `python -m tools.pipette.subagent_stop` with timeout 600 (must cover the worst-case 4 LLM attempts × 120s + git/parse overhead). Preserve existing PostToolUse and SessionStart blocks."*

- [ ] **Step B3.d: Verify settings round-trips**
...
```bash
git add tools/pipette/subagent_stop.py tests/pipette/test_subagent_stop_wiring.py .claude/settings.json
