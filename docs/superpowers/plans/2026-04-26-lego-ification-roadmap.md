...
- 2.3 — Begin intra-`app.js` extracts: API client first (cleanest leaf), then persistence, then drill state reducer.
  ...
  - 2.3.2 — ⏭ **Skipped (premise was wrong).** The roadmap said "extract `concept-storage.js`" assuming localStorage was scattered through app.js. Re-grep on 2026-04-27 showed concept storage is already a module (`public/js/store.js` with `loadConcepts`, `saveConcepts`, `getActiveId`, `setActiveId`, etc., owning `learnops_concepts` + `learnops_active`). Per-node drill state lives **inside** `concept.graphData`, not as a separate key. Other localStorage keys (`learnops_repair_reps_v1`, `learnops-theme`, `socratink:firstColdAttemptCreedSeen:v1`, legacy `learnops-state` migration, `gemini_key` reads) are scattered but small (2-3 lines each) and don't block the game-loop work. Treat as superseded.
  - 2.3.3 — Drill state reducer (pure state transitions — `drill-state.js`). Still relevant if/when drill state grows further; not blocking either.
