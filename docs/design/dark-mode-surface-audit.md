...
> **Superseded (commit a6de9bf, 2026-05-06).** The fix that shipped is **Pattern A graphite unification**, not the near-black indigo (`#0e0b1a` / `#1a1730`) proposed in section (4) below. The dark-mode tokens in `public/css/variables.css` now resolve to a graphite scale: `--surface-page-theme: #18181b`, `--surface-card-theme: #27272a`, `--surface-high: #3f3f46`, `--surface-highest: #27272a`, with regenerated `--gradient-card-*` pairs and a page-background gradient that no longer carries the violet radial blooms. Treat the primitives table, failure-mode diagnosis, and proposed systemic fix in this document as a historical record of the problem space — they do not describe what is in the codebase today.
>
> **Further obsoleted (2026-05-11, strip-as-nav port).** The Cytoscape graph view was removed. `.graph-detail`, `.graph-stage-wrap`, `.graph-detail-disclosure`, `.graph-detail-surface`, `.drill-ui-embedded`, the `--graph-tile-*` token family, and the `graph-repair-*` cyan animations no longer exist (most of `public/css/layout.css` was deleted in the same change). The dark-mode follow-ups in section (4) that target those selectors are obsolete; the live concept-view surfaces are now `public/css/concept-page.css` (strip + concept page) and `public/css/drill-chamber.css` (drill chamber). The `ai-runs-dashboard.css:354` bar-track and `login.html` follow-ups may still apply; verify against current code before acting.
