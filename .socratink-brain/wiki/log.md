...
## [2026-04-18] ingest | Dunlosky 2013 monograph + low-utility-techniques finding (first theta→brain handoff test)
First end-to-end test of the new theta-research → socratink-brain handoff contract. Theta read `dunlosky2013.pdf` (Psychological Science in the Public Interest 14(1), 4–58), produced an evidence-template paper note at `.agents/skills/theta-research/references/paper-notes/dunlosky2013.md` and a copy at `raw/research-notes/dunlosky2013.md`, and emitted a brain_handoff block proposing page_type=finding for low-utility techniques (the novel doctrine gap). Socratink Brain ingested: created `wiki/sources/research-note-dunlosky2013.md` (source_kind: research-note, basis: sourced, log_surface: none) citing the raw artifact; created `wiki/records/finding-low-utility-techniques-to-avoid.md` (basis: sourced, confidence: high) naming the five LOW-utility techniques (rereading, highlighting, summarization, keyword mnemonic, imagery for text) and translating each into concrete product constraints (no rereading surfaces in post-drill review, no highlight/annotation as study mechanic, no AI summaries as default "learn this node" action, no marketing copy promising highlight-based learning). Reinforces existing testing-effect + spacing-effect + generation-effect doctrine. Stats: 50 → 52 curated pages.
