...
Bloom's Taxonomy provides the cognitive vocabulary for what Socratink's drill is actually measuring:

- **Cold attempts test generation, not recall** — when a learner writes what they know about a node before studying, they're operating at Understand/Apply level (explain, construct), not just Remember. This is why cold attempts are more valuable than flashcard-style recognition.
- **Socratic questioning targets higher Bloom's levels** — the AI doesn't ask "what is X?" (Remember). It asks "why does X happen?" (Understand), "what would change if Y?" (Analyze), "is this explanation complete?" (Evaluate). The drill naturally pushes learners up the hierarchy.
- **Node mechanisms map to Bloom's levels** — different node types (definition, process, causal mechanism, comparison) implicitly demand different cognitive levels. A definition node might bottom out at Understand; a mechanism node requires Analyze.
- **Scoring rubrics need Bloom's awareness** — when the AI evaluates whether a learner's response is sufficient, it's implicitly judging Bloom's level. A learner who can Remember all the parts but can't Analyze their relationships hasn't actually learned the mechanism.
- **The `primed → drilled` transition is a Bloom's level shift** — primed means the learner has studied (Remember/Understand). Drilled means they've generated and been scaffolded through higher-level retrieval. The state transition encodes cognitive depth, not just exposure.

## Connections

- Desirable difficulty — higher Bloom's levels introduce productive struggle
- Zone of Proximal Development — Bloom's levels help locate the ZPD boundary for a given node
- Retrieval practice — retrieval at higher Bloom's levels produces stronger learning

## Sources

(No raw sources ingested yet — seed from Anderson & Krathwohl 2001, "A Taxonomy for Learning, Teaching, and Assessing")
