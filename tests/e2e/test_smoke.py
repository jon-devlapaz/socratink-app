...
def test_first_run_guidance_is_inline_not_modal(clean_page: Page, base_url: str) -> None:
    ...
    expect(clean_page.locator("#ignition-first-use")).to_have_text(
        "Give Socratink one source and one explanation target. Then the source closes."
    )
...
def test_start_learning_enters_source_hidden_reconstruction_from_product_flow(
    page: Page, base_url: str
) -> None:
    """Start learning closes the source before preserving the learner's explanation."""
    _enter_app_shell_as_guest(page, base_url)
    _wait_for_app_settled(page)
    source = "Vaccination leaves memory B and T cells that respond faster later."
    target = "Explain why a second exposure produces a faster immune response."
    explanation = "  The first exposure leaves memory cells ready to respond again.  "
    repair = "  Memory cells persist and activate faster when the antigen returns.  "
    extract_requests: list[str] = []
    page.on(
        "request",
        lambda request: extract_requests.append(request.url)
        if request.method == "POST" and request.url.endswith("/api/extract")
        else None,
    )

    page.locator("#nav-ignition").click()
    page.locator("#hero-single-input-field").fill(source)
    page.locator("#hero-cold-guess-field").fill(target)
    expect(page.locator("#hero-door-submit")).to_be_enabled()
    page.locator("#hero-door-submit").click()

    expect(page.locator("#hero-single-input")).to_be_hidden()
    expect(page.locator("#north-star-reconstruction")).to_be_visible()
    expect(page.locator("#north-star-target-text")).to_have_text(target)
    assert source not in page.locator("body").inner_text()
    assert extract_requests == []

    page.locator("#north-star-explanation-field").fill(explanation)
    page.locator("#north-star-reconstruction-submit").click()
    expect(page.locator("#north-star-saved")).to_be_visible()
    assert page.locator("#north-star-saved-explanation").text_content() == explanation
    expect(page.locator("#north-star-saved")).to_contain_text("not proof of mastery")
    expect(page.locator("#north-star-repair-form")).to_be_visible(timeout=20_000)
    expect(page.locator("#north-star-gap-text")).not_to_have_text(
        "Finding one consequential gap…"
    )

    page.locator("#north-star-repair-field").fill(repair)
    page.locator("#north-star-repair-submit").click()
    expect(page.locator("#north-star-repair-saved")).to_be_visible()
    assert page.locator("#north-star-repair-saved-text").text_content() == repair
    assert page.evaluate(
        "sessionStorage.getItem('socratink:north-star-session:v1') !== null"
    )
