...
The Socratinker is Socratink's internal product-memory skill. It turns scattered product evidence into durable, inspectable memory so future sessions do not re-derive the same product truth.
...
The Socratinker recognizes six operations:
...
- "Feed this artifact to Socratinker."
- "Ask Socratinker what we know about spacing."
- "Have Socratinker evaluate these drill logs."
- "Run a Socratinker health check."
- "Promote this only if it affects the MVP gate."

## Active Queue

Use `.socratinker/ACTIVE.md` only for validated, release-relevant work.
...
```bash
python3 -B .agents/skills/the-socratinker/scripts/validate_wiki.py .socratinker
python3 -B .agents/skills/the-socratinker/scripts/validate_wiki.py .agents/skills/the-socratinker/fixtures/healthy-kb
python3 -B .agents/skills/the-socratinker/scripts/wiki_stats.py .socratinker
