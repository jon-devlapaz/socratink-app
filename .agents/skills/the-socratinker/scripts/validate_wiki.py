#!/usr/bin/env python3
"""Validate structural integrity of a The Socratinker knowledge base.
...
def validate_page_frontmatter(relpath: str, frontmatter: dict, content: str, result: Result) -> None:
    required_fields = {"title", "type", "updated", "sources", "related", "basis", "workflow_status", "flags"}
    ...
    if page_type == "source":
        ...
        if "raw_artifacts" not in frontmatter:
            result.add_issue("warning", f"{relpath} source page missing raw_artifacts")
...
def validate_log_coverage(log_coverage_path: Path, result: Result) -> None:
...
def validate(kb_root: Path) -> Result:
    ...
    validate_log_coverage(wiki_dir / "log-coverage.md", result)
