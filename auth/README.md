...
- **Local dev guest fallback is intentional and loopback-only.** With `SOCRATINK_DEV_AUTOGUEST=1` or `SOCRATINK_LOCAL_AUTH_BYPASS=1`, unauthenticated protected GETs from loopback/testclient redirect through `/auth/guest`; if Supabase anonymous sign-in is unavailable, `/auth/guest` may mint a sealed local guest session via `build_local_dev_guest_session()`. This is distinct from `/auth/e2e/guest` and must stay disabled outside local dev.
