...
export async function runSedaLoop({
  ...
  while (phase) {
    ...
    if (lastEventType === "bridge_error") {
      const action = lastEvent.action || "bridge";
      const detail = lastEvent.message || lastEvent.error || "bridge call failed";
      if (options.loopUi) {
        console.error(`[Bridge error] ${action}: ${detail}`);
      } else {
        console.error(
          `\nBridge failed (${action}): ${detail}. Check GEMINI_API_KEY in .env or set SOCRATINK_TUI_FAKE_LLM=1 for local fixture mode.\n`,
        );
      }
    }
