...
function substrateConfirmedEvent(adequacy, decision = {}) {
  return eventBuilders.substrateConfirmed({
    adequacy,
...
async function callSubstrateBridge({
  ...
  const invalid = validateSubstrateGatePayload(payload);
  if (invalid) {
    return {
...
function appendSeed(events, decision) {
  ...
  events.push(
    eventBuilders.substrateSeedOffered({
      seed,
...
function appendSupportExhausted(events, decision) {
  ...
  events.push(
    eventBuilders.substrateSupportExhausted({
...
export async function handleSubstrateGate({
  ...
  if (!seedEvent && !refinementEvent) {
    ...
    if (gateResult.bridgeError) {
      events.push(gateResult.bridgeError);
      return { llm_calls: llmCalls };
    }
    const { decision, llmCall } = gateResult;
    llmCalls.push(
  ...
  if (!refinementEvent) {
    ...
    events.push(
      eventBuilders.substrateRefinement({
        text: refinement,
      }),
    );
