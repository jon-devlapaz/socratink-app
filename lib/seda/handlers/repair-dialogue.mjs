...
import { buildContingentProbe } from "../repair-scaffold.mjs";
...
export async function handleRepairDialogue({ events, bridge, prompt, options, ctx }) {
  ...
  if (isUncertainRepair(repair)) {
    ...
    if (uncertainDecision.action === "escalate") {
      ...
      state.isFirstTurn = false;
      ctx.composerCta = {
        label: repairTurnLabel(turnIndex + 1),
        text:
          ctx.repairScaffold.analogical_prompt ||
          ctx.repairScaffold.socratic_question ||
          "",
  ...
  const judge = applyRepairTurnBudget(dialogue.repair_dialogue, {
    turnIndex,
    maxRepairTurns: MAX_REPAIR_TURNS,
    repairText: repair,
    previousRepairText: lastDialogueTurn?.text,
  });
  ...
  if (!judge.bridge_ready) {
    const followUpPrompt = repairFollowUpPrompt({
      repair,
      judge,
      scaffold: ctx.repairScaffold,
    });
    if (followUpPrompt) {
      ...
      if (!options.loopUi) console.log(followUpPrompt);
    } else {
  ...
  state.queuedPrompt = repairFollowUpPrompt({
    repair,
    judge,
    scaffold: ctx.repairScaffold,
  });
