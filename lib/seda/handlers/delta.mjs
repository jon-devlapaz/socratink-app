...
import {
  ...
  invalidBridgeError,
  resultToBridgeError,
  validateRepairScaffoldPayload,
  validateSocraticDrillPayload,
...
export async function handleDelta({ events, derived, store, bridge, options, ctx }) {
  ...
  const zeroSchemaRepair = ctx.zeroSchemaCold;
  ...
  if (!scaffoldCallResult.ok) {
    events.push(
      resultToBridgeError({
        result: scaffoldCallResult,
        action: "repair-scaffold",
        phase: "delta",
      }),
    );
  ...
  const scaffoldResult = scaffoldCallResult.payload;
  const scaffoldInvalid = validateRepairScaffoldPayload(scaffoldResult);
  if (scaffoldInvalid) {
    events.push(
      invalidBridgeError({
        action: "repair-scaffold",
        phase: "delta",
        reason: scaffoldInvalid,
      }),
    );
    return { llm_calls: [] };
  }
  ...
  if (!drillCallResult.ok) {
    events.push(
      resultToBridgeError({
        result: drillCallResult,
        action: "socratic-repair-drill",
        phase: "delta",
      }),
    );
    return { llm_calls: [deltaCall] };
  }
  const drillResult = drillCallResult.payload;
  const drillInvalid = validateSocraticDrillPayload(drillResult);
  if (drillInvalid) {
    events.push(
      invalidBridgeError({
        action: "socratic-repair-drill",
  ...
  ctx.repairScaffold = applySocraticRepairDrillQuestion(scaffold);
  ctx.repairScaffold = {
    ...ctx.repairScaffold,
    socratic_question: buildRepairOpening({
      coldAttemptText: ctx.coldAttemptText,
      evaluation: ctx.coldEval,
      scaffold: ctx.repairScaffold,
      firstNode: ctx.firstNode,
      zeroSchemaCold: zeroSchemaRepair,
    }),
  };
  console.log("Repair one missing link.");
  console.log("Use your own words.");
  console.log("");
  console.log(ctx.section("repair", "Socratic Repair Drill"));
  ctx.composerCta = {
    label: "Repair",
    text: ctx.repairScaffold.socratic_question || "",
  };
  if (!options.loopUi) {
  ...
  events.push(eventBuilders.gapIdentified({
    surface: "delta",
    gap_id: ctx.gapId,
