  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case "target_submitted":
        requirePersistedFields(event);
        ctx.explanationTarget = event.text;
        break;
      case "initial_reconstruction_submitted":
        requirePersistedFields(event);
        ctx.initialReconstruction = event.text;
        ctx.initialReconstructionAt = event.at;
        break;
      case "initial_repair_submitted":
        requirePersistedFields(event);
        ctx.initialRepair = event.text;
        ctx.initialRepairAt = event.at;
        break;
