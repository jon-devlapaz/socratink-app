...
export const DIRECT_PHASE = {
  ...
  target_submitted: "initial_reconstruction",
  initial_reconstruction_submitted: "idle",
  initial_repair_submitted: "idle",
...
export function nextPhase(events) {
  ...
  const t = last.type;
  if (t === "initial_reconstruction_submitted") {
    return "reconstruction_gap";
  }
  if (
    (t === "gap_identified" && last.surface === "north_star_reconstruction")
    || (t === "bridge_error" && last.phase === "reconstruction_gap")
  ) {
    return "reconstruction_gap";
  }
  if (t === "cold_attempt") {
