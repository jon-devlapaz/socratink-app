...
export async function advanceSession(session, userText) {
  ...
  return withConsoleCapture(transcript, async () => {
    const rawUserText = userText == null ? "" : String(userText);
    const trimmedUserText = rawUserText.trim();
    ...
    if (
      ...
      session.pendingInput = isContentEntry
        ? rawUserText
        : trimmed;
    }
...
export function sessionResponse(session, delta) {
  ...
  return {
    ...
    phase: session.phase,
    awaiting,
    savedReconstruction: savedReconstructionResult(session.events),
    reconstructionRepair: reconstructionRepairResult(session.events, awaiting),
    ...
    events: learnerFacingEvents(session.events),
...
export function savedReconstructionResult(events = []) {
  const target = events.findLast((event) => event?.type === "target_submitted");
  ...
  return {
    target: target.text,
    explanation: attempt.text,
    submittedAt: attempt.at,
  };
}

export function reconstructionRepairResult(events = [], awaiting = null) {
  const gap = events.findLast(
    (event) =>
      event?.type === "gap_identified"
      && event?.surface === "north_star_reconstruction",
  );
  const repair = events.findLast(
    (event) => event?.type === "initial_repair_submitted",
  );
  const failure = events.findLast(
    (event) =>
      event?.type === "bridge_error"
      && event?.phase === "reconstruction_gap",
  );
  if (!gap && failure && awaiting?.key === "retry_reconstruction_gap") {
    return {
      status: "unavailable",
      retryable: true,
      message: "The gap could not be generated. Your explanation is still saved.",
    };
  }
  if (!gap) return null;
  return {
    status: repair ? "saved" : "ready",
    gap: gap.cue,
    gapId: gap.gap_id,
    repair: repair?.text ?? null,
    submittedAt: repair?.at ?? null,
  };
}

function learnerFacingEvents(events = []) {
  return events.map((event) => {
    if (event?.type === "source_submitted") {
      const { text: _source, ...safe } = event;
      return safe;
    }
    if (
      event?.type === "gap_identified"
