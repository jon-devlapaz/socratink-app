...
export function preflightTuiPaths(paths) {
  const requirePython = !(process.env.VERCEL || process.env.VERCEL_ENV);
  const checks = [
    {
      ok: !requirePython || fs.existsSync(paths.python),
      message:
    ...
    },
    {
      ...
      message:
        ...
        `Restore vendor/python from the loop runtime bundle if it is missing.`,
    },
    {
      ok: fs.existsSync(paths.trainingStorePath),
      message:
        `training-store.js not found at ${paths.trainingStorePath}. The vendored ` +
        `graph-truth canon is missing from the loop runtime bundle.`,
    },
    {
  ...
  if (failures.length) {
    throw new Error(
      `Socratink loop runtime preflight failed:\n  - ${failures.join("\n  - ")}`,
    );
  }
}
