...
const GraphEngine = (() => {
  ...
  function init() {
    // Tear down anything from a prior extraction — the if(!canvas) guard was the
    // bug: stale canvas refs kept old position:fixed styles alive across calls.
    ...
    // Graph must render inside the extraction overlay — never as a body-level element.
    // The overlay is position:fixed;inset:0 which makes it a containing block for
    // position:absolute children. This keeps the graph self-contained.
    const overlay = document.getElementById('extract-overlay');
    if (!overlay) {
      console.error('[GraphEngine] #extract-overlay not in DOM — cannot mount canvas.');
      return;
    }

    canvas = document.createElement('canvas');
    canvas.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;z-index:5;pointer-events:none;background:transparent;opacity:1;transition:opacity 0.6s ease;';
  ...
  function loop(now) {
    ...
    for(let n of nodes) {
      ...
      const labelAlpha = Math.min(1, t * 1.5);
      ctx.globalAlpha = labelAlpha;
      // Dark halo behind label text for legibility on any background
      ctx.shadowColor = 'rgba(10,8,22,0.9)';
      ctx.shadowBlur = 6;
  ...
  function stop() {
    ...
    if (canvas) {
      ...
      // The overlay's own removeOverlay() call takes the canvas with it when
      // it removes the parent from DOM. We just null the ref so init() is clean.
      setTimeout(() => { canvas = null; }, 700);
    }
  }
...
async function performAIExtraction(text, onSuccess, onError, onPhaseChange) {
  ...
  function log(msg) { console.log('[AIExtract]', msg); }
...
function startSettings() {
  ...
  triggerArea.innerHTML = `
    <div class="settings-panel">
      ...
      <div class="settings-box" style="margin-top:20px;">
        <div class="settings-section-header">
          <h4>Phase 1: Staging Output</h4>
          <span class="settings-dot" id="fs-dot"></span>
        </div>
        <p class="settings-subtext" style="margin-bottom:12px;">Select the local Obsidian Vault "Staging" folder where Phase 1 Knowledge Maps will be written.</p>
        <div class="settings-actions" style="margin-top:0;">
          <button id="fs-btn" class="settings-save">Select Directory</button>
        </div>
        <div id="fs-status" class="settings-status"></div>
      </div>
    </div>
  `;
  ...
  // FS UI logic
  const fsBtn = triggerArea.querySelector('#fs-btn');
  const fsDot = triggerArea.querySelector('#fs-dot');
  const fsStatus = triggerArea.querySelector('#fs-status');

  async function updateFSUi() {
    if (await FileSystem.hasDirectorySelected()) {
      const name = await FileSystem.getDirectoryName();
      fsDot.classList.add('connected');
      fsDot.classList.remove('error');
      fsStatus.textContent = `Target: ${name}`;
      fsStatus.style.color = 'var(--text-sub)';
      fsBtn.textContent = 'Change Directory';
    } else {
      fsDot.classList.remove('connected');
      fsDot.classList.add('error');
      fsStatus.textContent = 'No directory path configured.';
      fsStatus.style.color = 'var(--danger)';
      fsBtn.textContent = 'Select Directory';
    }
  }
  ...
  fsBtn.addEventListener('click', async () => {
    fsBtn.disabled = true;
    fsBtn.textContent = 'Picking...';
    try {
      const ok = await FileSystem.initDirectoryPicker();
      if (ok) {
        await updateFSUi();
        fsStatus.textContent += ' (Saved)';
        fsStatus.style.color = 'var(--success)';
      } else {
        await updateFSUi();
      }
    } catch (err) {
      console.error(err);
      fsStatus.textContent = 'FS error: ' + err.message;
      fsStatus.style.color = 'var(--danger)';
    } finally {
