#!/usr/bin/env python3
"""Python LLM bridge for the vendored Socratink loop runtime.

The loop runtime stays in Node so it can reuse the browser training-store and
training-derive modules under lib/canon/. This bridge keeps LLM calls on the
vendored Python seam under vendor/python/.
"""
...
if not hasattr(ai_service, "_normalize_drill_evaluation"):
    raise RuntimeError(
        "vendored ai_service is missing _normalize_drill_evaluation; the "
        "loop bridge depends on this evaluation-normalization seam."
    )
